...
### サムネの作り方（2026-08-09 に本人が決定）

**16:9のタイトル入りサムネは、素材のまま4:5で切り出す。合成や文字の組み直しはしない。**

- **文字は見切れてよい。** 一覧のカードには作品名がテキストで表示されるので（`works.html` の `.work-item-title`）、画像内の文字を読ませる必要はない。デザインの気配が残っていればよい
- 切る位置は**被写体が欠けないところ**を選ぶ。中央固定にしない
- **素材の黒フチは外す。** ただし黒フチの幅は一様でないことがあるので、**実際に切り出す横位置の範囲内で**上下の境界を測ること。画像全体で測ると数pxの黒帯が残る（TMCPで実際に起きた）
- 合成して文字を小さく入れ直す案は試したが、臨場感が落ちるとして不採用

デザインした16:9のタイトル入りサムネはYouTube側で本来の形のまま見えるので、役割を分けている。
